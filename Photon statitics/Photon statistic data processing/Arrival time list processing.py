...
def gen_hist_csv_from_dts(d2, res=0.4, t_range=25100, chA='ch0', chB='ch1'):
	d3 = d2 + r"\dts " + chA + " & " + chB
	os.chdir(d3)
	a = re.findall('\d+', chA)[0]
	b = re.findall('\d+', chB)[0]

	# datafiles = glob.glob(d3 + r'\dts_chs' + a + b + r'*')
...
def unwrap_4ch_data(d0):
	...
	for i0, v0 in enumerate(fs):
		...
		with open(v0, 'r') as input_file:
			while current_slice < tot_lines / lines_per_slice:
				print('lines', current_slice * lines_per_slice,
					  'to', (current_slice + 1) * lines_per_slice,
					  'of', tot_lines)
				data = list(islice(input_file, lines_per_slice))
				data = list(map(int, data))
				data_64 = np.asarray(data, dtype=np.uint64)
				
				# differentiate to get reset points
				ddata = np.diff(np.asarray(data))
				# locate clock reset values
				resets = find_clk_resets(ddata)
				cycles = len(resets)
				
				# begin reorganising datasets
				print('unwrapping', cycles, 'cycles in ch', i0)
				current_slice += 1
				print('processing lsts, slice', current_slice, 'of',
					  int(tot_lines / lines_per_slice))
				
				# use unwrap_data function to reorganise dataset 
				data_out = unwrap_data(data_64, resets)

				# save 2d array of arrival times  to file
				fname = 'ch' + str(i0) + ' slice ' + str(current_slice)
