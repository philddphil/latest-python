...
# Difference in radius of curvature:
ax1, fig1 = set_figure('dRs mean', 'L / μm', 'dν / MHz', 2.2)
ax2, fig2 = set_figure('dRs positive', 'L / μm', 'dν / MHz', 2.2)
dRs = np.linspace(200e-6, 240e-6, 8)
dnus = []
for i9, v9 in enumerate(dRs):
    print(i9, v9*1e6, 'μm RoC')
    R = v9
    ...
    for i0, v0 in enumerate(Ls[:]):
        ...
        for i1, v1 in enumerate(ns_1033[:]):
            ...
            if curved == True:
                ...
                nu_1377_L = mode_nu(n_1377_loop,v0,g2,g2)  
            
            else:
                ## planar cavity formula
                nu_1033_L = mode_nu_approx(n_1033_loop,v0)
                nu_1377_L = mode_nu_approx(n_1377_loop,v0)
...
PPT_save_plot(fig1, ax1, 'detunings with RoC minus mean', 600, False)
PPT_save_plot(fig2, ax2, 'detunings with RoC pos', 600, False)
