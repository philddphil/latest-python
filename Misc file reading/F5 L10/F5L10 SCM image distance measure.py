...
def PPT_save_2d_im(fig, ax, cb, name, dpi=600):
    ...
    plt.setp(cbylabel_obj, color='xkcd:black')
...
def gin(x, y, n=0, title=''):
    set_figure('image to measure', 'x', 'y')
...
pY = (r"C:\local files\Experimental Data\F5 L10 Confocal measurements"
      r"\SCM Data 20210707\Raster scans")
...
x, y, img = load_SCM_F5L10(f)
im_min = np.min(np.min(img))
im_mean = np.mean(img)
lb = os.path.basename(f)

ax0, fig0, cs = set_figure('image to measure', 'x', 'y')
...
ax0.set_title('left click to place points, right click to end')
pts = np.asarray(plt.ginput(0, timeout=0))
plt.close()
print('distance between point pairs')
ax1, fig1, cs = set_figure('image to measure', 'x', 'y')
...
cbar.set_label('cps', rotation=270, c='xkcd:black')
cbar.ax.minorticks_off()
ls = []
for i0, v0 in enumerate(pts):
    if i0 % 2 == 0:
        l = np.sqrt((v0[0] - pts[i0 + 1][0])**2 + (v0[1] - pts[i0 + 1][1])**2)
        ax1.plot([v0[0], pts[i0 + 1][0]], [v0[1], pts[i0 + 1][1]], ':')
        ls.append(l)
        x_txt = v0[0] - (v0[0] - pts[i0 + 1][0]) / 2
        y_txt = v0[1] - (v0[1] - pts[i0 + 1][1]) / 2
        ax1.text(x_txt, y_txt, str(int(np.round((i0 / 2) + 1))), c='xkcd:black')
        print('line', int(i0 / 2 + 1), '--' ,np.round(l, 2))
    else:
        pass
print(np.mean(ls))
plt.show()
PPT_save_2d_im(fig1, ax1, cbar, 'lines measured')
